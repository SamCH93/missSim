...
format:
  html:
    ...
    code-links:
      - text: Source Code
        icon: file-code
        href: https://github.com/SamCH93/missSim
execute:
  message: false
  warning: false
---
...
n_quantified <- data_lr |> 
  filter(q1_is_sim_study == "yes") |> 
  filter(q2_mentions_missingness == "yes") |> 
  count(q2_2_missingness_summarized) |> 
  filter(!q2_2_missingness_summarized == "not quantified") |>
  pull(n) |> 
  sum()
...
q2_2_plot
```
This means that out of the `r n_mentions` studies mentioning missingness, `r n_quantified` studies quantified missingness. 

Or, as a table with percentages as reported in the manuscript:
```{r}
data_lr |>
  filter(q1_is_sim_study == "yes") |> 
  filter(q2_mentions_missingness == "yes") |> 
  ...
  rename("Missingness Summary" = q2_2_missingness_summarized) |> 
  knitr::kable()
```
...
We first decided that studies that explicitly report that no missingness occurred get their own category for question 3:
```{r}
data_lr <- data_lr |> 
    mutate(q3_report_dealing_with_missingness = if_else(q2_2_missingness_summarized == "zero missingness",
                                                      "yes\n(zero)",
...
```{r, echo = FALSE}
# count number of studies that mention missingness
q3_yes_count <- q3_nonmention_counts |> 
  filter(q3_report_dealing_with_missingness == "yes") |> 
  pull(n)

q3_yeszero_count <- q3_nonmention_counts |> 
  filter(q3_report_dealing_with_missingness == "yes\n(zero)") |> 
  pull(n)

q3_yesoverall_count <- q3_yes_count + q3_yeszero_count

```


Overall, `r q3_yesoverall_count` studies mentioned missingness and reported how they handled it, including `r q3_yeszero_count` studies that explicitly reported that no missingness occurred. 
...
```{r, echo = FALSE}
# calculat number of remaining studies for this figure
num_left_q31 <- data_lr |>
  filter(q1_is_sim_study == "yes") |>
  filter(q2_mentions_missingness == "yes") |>
  filter(
    q3_report_dealing_with_missingness == "yes" |
      q3_report_dealing_with_missingness == "unclear"
  ) |> 
  count() |> 
  pull(n)
```
...
If authors report how they dealt with missingness, what method did they use? Here, we are looking at `r num_left_q31` studies where missingness occurred and some form of missingness handling was documented, even if it was (partly) unclear.
...
Alternatively, reported in a table with percentages as reported in the manuscript:
```{r}
data_lr |> 
  filter(q1_is_sim_study == "yes") |>
  filter(q2_mentions_missingness == "yes") |>
  ...
  mutate(percentage = paste0( round(n/sum(n)*100, 1), "%")) |>
  rename("Method" = q3_1_method_dealing_with_missingness) |>
  knitr::kable()
```
...
```{r, echo = FALSE}
# count studies that report dealing with missingness
q3_2_yes_count <- data_lr |> 
  filter(q1_is_sim_study == "yes") |>
  filter(q2_mentions_missingness == "yes") |>
  filter(q3_report_dealing_with_missingness == "yes" | q3_report_dealing_with_missingness == "unclear")  |>
  filter(!is.na(q3_2_method_justification)) |>
  filter(q3_2_method_justification == "yes") |> 
  count() |> 
  pull(n) 

q3_2_unclear_count <- data_lr |> 
  filter(q1_is_sim_study == "yes") |>
  filter(q2_mentions_missingness == "yes") |>
  filter(q3_report_dealing_with_missingness == "yes" | q3_report_dealing_with_missingness == "unclear") |>
  filter(!is.na(q3_2_method_justification)) |>
  filter(q3_2_method_justification == "unclear") |> 
  count() |> 
  pull(n)

q3_2_all_count <- q3_2_yes_count + q3_2_unclear_count
```


If authors report how they dealt with missingness, what justification did they provide for their choice? We filter out one study here where missingness was considered as the outcome, so there is no justification needed. Overall, `r q3_2_all_count` studies some form of justification of their choice, including `r q3_2_unclear_count` studies that provided an unclear justification. In the following, we agreed to count NA in `q3_2_method_justification` as no.
...
q3_2_plot <- data_lr |> 
  ...
  filter(q2_mentions_missingness == "yes") |>
  filter(q3_report_dealing_with_missingness == "yes" | q3_report_dealing_with_missingness == "unclear") |>
    mutate(q3_2_method_justification = if_else(is.na(q3_2_method_justification), "no", 
...
Again, a table with percentages:
```{r}
data_lr |> 
  filter(q1_is_sim_study == "yes") |>
  filter(q2_mentions_missingness == "yes") |>
  ...
  mutate(q3_2_method_justification = if_else(is.na(q3_2_method_justification), "no", 
                                             q3_2_method_justification)) |>
  count(q3_2_method_justification) |> 
  mutate(percentage = paste0( round(n/sum(n)*100, 1), "%")) |>
  rename("Justification" = q3_2_method_justification) |>
  knitr::kable()
```
...
If authors provide a justification for their choice of handling missingness, what type of justification do they provide? This includes all studies with a clear or unclear justification as long as some form of justification is provided.  
```{r}
...
data_lr |> 
  ...
  count(q3_3_type_justification) |> 
  mutate(percentage = paste0( round(n/sum(n)*100, 1), "%")) |>
  rename("Justification Type" = q3_3_type_justification) |>
  knitr::kable()
```
...
```{r, echo=FALSE}
# code availability in percentage
code_available_pct <- data_lr |> 
  filter(q1_is_sim_study == "yes") |> 
  count(q4_code_available) |> 
  mutate(percentage = paste0( round(n/sum(n)*100, 1), "%")) |> 
  filter(q4_code_available == "yes") |>
  pull(percentage)
```


This means that, overall, `r code_available_pct` of the simulation studies provided code.
...
Or as a table with percentages:

```{r}
data_lr |> 
  filter(q1_is_sim_study == "yes") |> 
  count(journal, q4_code_available) |> 
  group_by(journal) |> 
  mutate(prop = n/sum(n)) |> 
  mutate(percentage = paste0(round(prop*100, 1), "%")) |> 
  select(!prop) |> 
  rename("Code Availability" = q4_code_available) |> 
  knitr::kable()
``` 
...
During the revision of the manuscript, thanks to the helpful comment of a reviewer, we renamed "case-wise" and "list-wise" deletion to "method-wise" and "repetition-wise" deletion, respectively. 
```{r}
# studies per journal (excluding "unclear")
q1_totals <- data_lr |> 
  filter(q1_is_sim_study != "unclear") |> 
  count(journal, name = "total")

q1_sim_counts <- data_lr |> 
  filter(q1_is_sim_study == "yes") |> 
  count(journal, name = "sim")

q1_labels <- left_join(q1_sim_counts, q1_totals, by = "journal") |> 
  mutate(label = paste0(sim, "/", total),
         prop = sim / total)
...
q1_plot_pub <- data_lr |> 
  ...
  ungroup() |> 
  filter(q1_is_sim_study == "yes") |> 
  ggplot(aes(x = journal, y = prop, fill = journal)) +
    geom_col(col = 1) +
    labs(x = NULL, 
         title ="Simulation studies per journal", 
         fill = "Journal") +
    # add raw numbers
  geom_text(data = q1_labels, 
          aes(x = journal, y = prop/2, label = label), 
...
q3_plot_nonmention_pub <- data_lr |>
  ...
  ggpubr::geom_bracket(xmin = 1.75, xmax = 5.25, 
               inherit.aes = FALSE, label = "If mentioned",
               y.position = 135, label.size = 5.8)
...
q3_1_plot_counts <- data_lr |> 
  ...
  mutate(
    q3_1_method_dealing_with_missingness = case_match(
      ...
      "case-wise deletion" ~ "method- wise",
      "list-wise deletion" ~ "repetition- wise",
      .default = q3_1_method_dealing_with_missingness
    )
  ) |>
  count(q3_1_method_dealing_with_missingness) |>
...
q3_1_plot_pub <- data_lr |>
  ...
  theme(legend.position = "none",
                             ...
                             labs(x = NULL,
                                  y = NULL,
                                  title = "Method of handling missingness")



# sim studies with known code availability per journal
q4_totals <- data_lr |> 
  filter(q1_is_sim_study == "yes") |> 
  filter(!is.na(q4_code_available)) |> 
  count(journal, name = "total")
...
q4_code_yes <- data_lr |> 
  filter(q1_is_sim_study == "yes", q4_code_available == "yes") |> 
  count(journal, name = "yes_count")

q4_labels <- left_join(q4_code_yes, q4_totals, by = "journal") |> 
  mutate(label = paste0(yes_count, "/", total),
...
For the first revision, we tried to change the first and last part of the plot to show absolut numbers instead of proportions, and add the total number of studies as a label. However, in the end, we kept the original proportion plot, but removed the shaded "no" area.
```{r}
# slightly hacky to get colors in correct order
cols_rev <- unname(cols)
cols_rev <- cols_rev[c(4,2,3,1)]

# get counts
q1_rev_plot_counts <- data_lr |> 
  count(journal, q1_is_sim_study) |> 
  # filter(q1_is_sim_study == "yes") |> 
  # add overall count to each journal
  group_by(journal) |> 
  mutate(n_journal = sum(n)) |> 
  ungroup() |> 
  mutate(journal = as.factor(paste(journal, "\n(n = ", n_journal, ")", sep = ""))) |> 
  filter(q1_is_sim_study == "yes")


q1_plot_rev1 <- data_lr |> 
  add_count(journal) |> 
  filter(q1_is_sim_study != "unclear") |> 
  filter(q1_is_sim_study != "no") |> 
  # add the count to the x axis label (journal column)
  mutate(journal = as.factor(paste(journal, "\n(n = ", n, ")", sep = ""))) |> 
  ggplot(aes(x = journal, 
             fill = journal)) +
  geom_bar(col = 1) +
    labs(x = NULL, 
         title ="Number of simulation studies per journal", 
         fill = "Journal") +
   # slightly hacky to get colors in correct order
    scale_fill_manual(values = cols_rev) +
    scale_alpha_manual(values = c(0.25, 1), guide = "none") +
    ...
    geom_text(data = q1_rev_plot_counts, 
              aes(x = journal, y = n, label = n), 
              size = 5.8, 
...
q4_plot_rev1 <- data_lr |> 
  filter(q1_is_sim_study == "yes") |>
  add_count(journal) |> 
  filter(!is.na(q4_code_available)) |>
  filter(q4_code_available == "yes") |> 
  mutate(journal = as.factor(paste(journal, "\n(n = ", n, ")", sep = "")) ) |>
  ggplot(aes(x = journal, fill = journal))+
  geom_bar(col = 1)+
  theme(legend.position = "none") + 
  scale_y_continuous(limits = c(0,100), expand = c(0,0)) +
  labs(x = "Journal", y = "Number of Studies with Code", fill = "Journal:")+
  scale_fill_manual(values = cols_rev) +
   geom_text(data = q4_counts_rev,
             aes(x = journal,
                 y = n_code,
                 label = n_code),
             vjust = -0.5,
...
Save plots:
```{r, eval = FALSE}
ggsave(here("literature-review/figures/fig_lit_review_results_rev2.pdf"), p_c_leg, width = 15*0.76, height = 10*0.76)
```
